"""
Standard figure panels for scQDiff drift field analysis.
All functions read from adata.uns[key] populated by sqd.tl.fit_drift.
"""
...
def _get(adata, key, field):
    if key not in adata.uns:
        raise KeyError(f"'{key}' not in adata.uns. Run sqd.tl.fit_drift first.")
    ...
    **scatter_kw,
    ...
    if velocity_key is None:
        if basis == "X_pca" and "X_velocity_pseudo" in adata.obsm:
            ...
            raise KeyError("No drift vectors found. Run sqd.tl.fit_drift first.")
...
def instability_genes(
    ...
    if not gene_names:
        raise ValueError("No gene names found. Re-run sqd.tl.fit_drift on an adata with PCA loadings.")
...
def summary_figure(
    ...
    fig.suptitle("scQDiff — operator-level view of single-cell dynamics",
