...
All functions read from adata.uns[key] populated by sqd.tl.fit_bridge.
"""
...
def _get(adata, key, field):
    if key not in adata.uns:
        raise KeyError(f"'{key}' not in adata.uns. Run sqd.tl.fit_bridge first.")
...
def bridge_source_target(adata, key="scqdiff_bridge", basis="X_pca",
                          ax=None, save=None):
...
def bridge_instability(adata, key="scqdiff_bridge", ax=None, save=None):
...
def bridge_archetypes(adata, key="scqdiff_bridge", axes=None, save=None):
...
def bridge_genes(adata, key="scqdiff_bridge", n_genes=15,
                 axes=None, save=None):
