    ...
    if src_group is not None and tgt_group is not None and groupby is not None:
        ...
        if time_key not in adata.obs.columns:
            raise KeyError(f"time_key='{time_key}' not in adata.obs. "
                           "Run sqd.pp.prepare_trajectory first.")
...
def fit_bridge(
    ...
    *,
    ...
    By pseudotime quantile (default)::
        ...
        sqd.tl.fit_bridge(adata, src_quantile=0.20, tgt_quantile=0.80)

    By cell-type label::

        sqd.tl.fit_bridge(adata, groupby='cell_type',
    ...
    if rep not in adata.obsm:
        raise KeyError(f"'{rep}' not in adata.obsm. Run sqd.pp.prepare_trajectory first.")
