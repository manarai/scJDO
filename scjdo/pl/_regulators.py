"""
Regulatory network figures for scJDO instability analysis.
...
def _table(adata, key="scjdo_regulators", direction=None) -> pd.DataFrame:
    tables = _get(adata, key, "tables", {})
    if not tables:
        raise KeyError(f"No regulator tables in adata.uns['{key}']. "
                       "Run sjd.tl.infer_regulators first.")
...
def regulator_heatmap(
    ...
    """
    df       = _table(adata, key, direction).head(n_show)
    src_key  = scjdo_key or _get(adata, key, "source_key", "scjdo")
...
def regulator_network(
    ...
    Solid edges  = reference database confirms TF → target AND target is a top
                   instability gene (known biology, validated by scJDO).
...
def regulator_summary(
    ...
    regulator_heatmap(adata, **kw, scjdo_key=scjdo_key,
                      n_show=n_show,                       ax=ax_b)
    regulator_scatter(adata, **kw, n_label=10,             ax=ax_c)
    regulator_profiles(adata, **kw, scjdo_key=scjdo_key,
                       n_tfs=3,                             ax=axes_d[0])
...
# ---------------------------------------------------------------------------
# Per-branch plot suite — encapsulates the 6-plot try/except loop pattern
# that was hand-written in Figure3_FA cell 15 and Figure5 cell 12.
# ---------------------------------------------------------------------------

_PANEL_FACTORY = {
    # name : (callable, default_kwargs, needs_scjdo_key)
    "barplot":  (regulator_barplot,   dict(n_show=20),                        False),
    "heatmap":  (regulator_heatmap,   dict(n_show=15),                        True),
    "scatter":  (regulator_scatter,   dict(n_label=10),                       False),
    "profiles": (regulator_profiles,  dict(n_tfs=3),                          True),
    "summary":  (regulator_summary,   dict(),                                 True),
    "network":  (regulator_network,   dict(n_tfs=5, n_targets=6, n_denovo=4), True),
}


def branch_regulator_panels(
    adata,
    branch_models,
    outdir: str,
    *,
    ...
    """
    Emit the standard per-branch regulator plot suite into per-branch subdirs.

    For each branch in ``branch_models``, looks up the regulator-result key
    ``f'{regulators_prefix}_{name}'`` and the drift/bridge source key
    ``f'{key_prefix}_{name}'``, then runs the requested ``panels`` and writes
    each to ``{outdir}/{name}/reg_{panel}.{file_ext}``. Each call is wrapped
    in try/except so a failure on one panel does not abort the rest.

    Parameters
    ----------
    adata : AnnData
        Must contain regulator-result entries written by
        :func:`infer_regulators_branches` (or per-branch calls to
        :func:`infer_regulators` whose results were copied back onto the
        full AnnData's uns).
    branch_models : dict
        ``{name: model}`` from :func:`fit_drift_branches` or
        :func:`fit_bridge_branches`.
    outdir : str
        Root directory; per-branch subfolders are created inside.
    panels : tuple of str
        Subset of ``{'barplot', 'heatmap', 'scatter', 'profiles',
        'summary', 'network'}``. Default = all six.
    direction : {None, 'forward', 'backward'}
        Which direction to plot for bridge results. ``None`` (default) plots
        the primary direction. Use ``'forward'``/``'backward'`` to pick one
        side of a bridge; pass the function twice to get both.
    key_prefix, regulators_prefix : str
        Prefixes (see :func:`infer_regulators_branches`).
    file_ext : str
        ``'pdf'`` or ``'png'``.
    panel_kwargs : dict, optional
        Per-panel kwarg overrides, e.g. ``{'barplot': {'n_show': 30}}``.

    Returns
    -------
    dict
        ``{branch_name: list_of_successful_panels}``.

    Examples
    --------
    >>> models = sjd.tl.fit_drift_branches(adata, branch_key='branch_masks')
    >>> sjd.tl.infer_regulators_branches(adata, models, organism='human',
    ...                                   min_targets=1, n_top=15)
    >>> sjd.pl.branch_regulator_panels(adata, models, 'results/figure3/')
    """
    import os
    ...
    for name in branch_models:
        ...
        drift_key = f"{key_prefix}_{name}"
        branch_dir = os.path.join(outdir, name)
        os.makedirs(branch_dir, exist_ok=True)

        reg_uns = adata.uns.get(reg_key)
        ...
        ok = []
        for panel in panels:
            if panel not in _PANEL_FACTORY:
                warnings.warn(
                    f"Unknown panel '{panel}'; valid: {list(_PANEL_FACTORY)}",
                    UserWarning, stacklevel=2,
                )
                continue
            fn, defaults, needs_scjdo = _PANEL_FACTORY[panel]
            tag = f"_{direction}" if direction else ""
            save_path = os.path.join(branch_dir, f"reg_{panel}{tag}.{file_ext}")

            kwargs = dict(defaults)
            if needs_scjdo:
                kwargs["scjdo_key"] = drift_key
            if direction is not None:
                kwargs["direction"] = direction
            kwargs.update(panel_kwargs.get(panel, {}))

            try:
                fn(adata, key=reg_key, save=save_path, **kwargs)
                ok.append(panel)
            except Exception as e:
                if verbose:
                    print(f"  [{name}] {panel}: {type(e).__name__}: {e}")

        summary[name] = ok
        if verbose:
            print(f"[{name}] saved {len(ok)}/{len(panels)} panels -> "
                  f"{branch_dir}  ({', '.join(ok)})")
